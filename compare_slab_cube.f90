...
program read_potential_files
  ...
  character*200, parameter :: cube_file_path = '/data/inspur_disk03/userdir/shliao/haoran_yunchong_IC/&
&yunchong_tng_IC/slab_to_cube/res/'
  ...
  integer*4 file_size, image_ngrid
  ...
  do file_i=1,file_num
    ...
    filename = slab_file_path(1:len_trim(slab_file_path)) // '/' // slab_file_basename(1:len_trim(slab_file_basename))&
& // '_potential.' // fnumber(1:1)
  ...
  do image_z=1,image_per_dim
    do image_y=1,image_per_dim
      do image_x=1,image_per_dim
        ...
        write(redshift, '(F7.3)') ic_redshift
        write(fnumber, '(I1)') image_count
        filename = cube_file_path(1:len_trim(cube_file_path)) // '/' // redshift(1:len_trim(redshift)) //&
& '_phi1_' // fnumber(1:1) // '.bin'
        print *, 'Reading', filename

        inquire(file=filename, size=file_size)
        image_ngrid = (file_size / 4)**(1./3.)
!        print *, image_ngrid
        allocate(phi_tmp_cube(image_ngrid, image_ngrid, image_ngrid))
        open(1, file=filename, access='stream')
        read(1) phi_tmp_cube
        ...
        do k=1, image_ngrid
          do j=1, image_ngrid
            do i=1, image_ngrid
              phi_cube((image_x-1)*image_ngrid+i, (image_y-1)*image_ngrid+j, (image_z-1)*image_ngrid+k) = phi_tmp_cube(i,j,k)
            end do
          end do
        end do

        close(1)
        deallocate(phi_tmp_cube)
      end do
    end do
  end do
  ...
  do k=1, ngrid 
    do j=1, ngrid
      do i=1, ngrid
        float_tmp = phi_slab(i, j, k)
        if(float_tmp .ne. phi_cube(i,j,k)) then
          print *, i, j, k, phi_slab(i, j, k), phi_cube(i, j, k)
        end if
      end do
    end do
  end do
